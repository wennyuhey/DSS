...
class Covpool(Function):
     ...
     def forward(ctx, input, target):
         ...
         target = target.reshape(batchSize,dim,M)
         I_hat = (-1./M/M)*torch.ones(M,M,device = x.device) + (1./M)*torch.eye(M,M,device = x.device)
         I_hat = I_hat.view(1,M,M).repeat(batchSize,1,1).type(x.dtype)
         y = x.bmm(I_hat).bmm(target.transpose(1,2))
         ctx.save_for_backward(input,I_hat,target)
         return y
     @staticmethod
     def backward(ctx, grad_output):
         input,I_hat,target = ctx.saved_tensors
         x = input
         batchSize = x.data.shape[0]
         dim = x.data.shape[1]
         h = x.data.shape[2]
         ...
         grad_input = grad_input.bmm(target).bmm(I_hat)
...
def CovpoolLayer(var, target):
    return Covpool.apply(var, target)
