dataset_type_t = 'CarCityscapesDataset'
dataset_type_s = 'CarCocoDataset'
data_root_s = 'data/sim10k/'
...
data_t = dict(
    ...
    train=dict(
        ...
        'annotations/instancesonly_car_gtFine_train.json',
        img_prefix=data_root_t + 'leftImg8bit/train/',
        pipeline=train_pipeline),
    val=dict(
        type=dataset_type_t,
        ann_file=data_root_t + 'annotations/instancesonly_car_gtFine_val.json',
        img_prefix=data_root_t + 'leftImg8bit/val/',
        pipeline=test_pipeline),
    test=dict(
        type=dataset_type_t,
        ann_file=data_root_t + 'annotations/instances_car_gtFine_val.json',
        img_prefix=data_root_t + 'leftImg8bit/val/',
        pipeline=test_pipeline))
